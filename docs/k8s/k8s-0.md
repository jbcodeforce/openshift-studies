...
* A **PersistentVolumeClaim (PVC)** is a request for storage by a user/app. It is a similar concept as a pod: Pods consume node
...
Different classes might map to different quality-of-service levels, and to backup policies. Each StorageClass contains
 the fields provisioner, parameters, and reclaimPolicy. vSphere, minio, GlusterFS, NFS are storage classes.
